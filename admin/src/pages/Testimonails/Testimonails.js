...
function Testimonails() {
    ...
    const getTestData = () => {
        axios.get('http://localhost:5002/api/testimonails')
            .then((response) => {
    ...
    const handleAddTestData = (e) => {
        ...
        if (testPositions.current.value.length === 0) {
        ...
        if (editDetails) {
            ...
            axios.put(`http://localhost:5002/api/testimonails/${testId}`, editedTestimonailData)
                .then((response) => {
                    alert("Testimonail edited successfully");
            ...
            axios.post('http://localhost:5002/api/testimonails', newTestimonailData)
                .then((response) => {
                    alert("Testimonail added successfully");
                    window.location.reload();
                    setIsTestAdded(true); // Assuming you're using this state for some purpose
                })
                .catch((error) => console.error("Error adding Testimonail:", error));
        }
    ...
    const handleDelete = (testId, e) => {
        if (window.confirm("Are you sure you want to delete the Testimonail?")) {
            axios.delete(`/api/testimonails/${testId}`)
                .then((response) => {
                                        ...
                                        {testData.length > 0 ? (
                                            testData.map((testimonial) => (
                                                <div key={testimonial._id}>
                                                    <TestimonailData
                                                        id={testimonial._id}
                                                        postImage={testimonial.postImage}
                                                        testTimestamp={testimonial.position}
